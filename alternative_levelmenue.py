...
while run:
  ...
  ################################################################################################################################## change
    if keys[pygame.K_SPACE]:
        if not space_pressed:               # Run this code only if space key is not already pressed = only once per press
            board = char_facing_board()     # Get the board that the character is facing
            if board is not None:
                subprocess.Popen(["python3", "quizz_app.py" , username, board.level])
...
##################################################################################################################################### block that i coppy out and push it in. The change is mybe a little bit smaler
